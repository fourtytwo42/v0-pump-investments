...
- Version 4.0.14 is deployed from immutable `main` release `9cb21305e30fb900cb96924fa0f00660c3fe51bf`. Newly active unknown Pump tokens now bypass the saturated periodic lifecycle backlog and use bounded single-token fallback when Pump's batch endpoint omits them.
- The previous lifecycle-correction v4.0.10 release was `f7266aced8cb92e2943ee784adb5f78fa84b324a`.
- `/home/hendo420/pumpInvestments/current` points to the immutable v4.0.14 release. Both PM2 services are online from that directory, exactly three releases are retained, and shared environment/spool/image/support/log state remains under `/var/lib/pump-investments`.
...
- V4.0.14 fixes newly active Pump tokens being hidden from verified-only Bonding results. Atomic ingestion immediately queues active unknowns at priority 110 (or 120 for migration hints), duplicate queue requests preserve retry backoff, hot reconciliation targets only stale recent rows, stale queue work is removed, and omitted batch results receive bounded single-token fallback.
- Live v4.0.14 verification found zero recently traded Pump/Pump-bonding tokens still `UNKNOWN`. A raw one-minute Bonding sample rose from 18 before deployment to 125 after deployment, while default filters rose from 8 to 43. Protected health showed about 1.4-second persisted lag, 14 high-priority checks due, zero dead-letter files, and lifecycle batches resolving 48-50 of 50 checks.
- A 2026-08-08 live 1-minute Bonding audit found healthy trade ingestion (about 553 trades/144 tokens per minute and about 1.6-second freshness) but an overloaded lifecycle verifier. Roughly 40 recently traded Pump/Pump-bonding tokens remained `UNKNOWN`; Pump's batch endpoint returned only one of 40 new mints while eight of eight sampled single-token lookups confirmed `program=pump, complete=false`. The queue had about 10,112 rows/9,773 due; priority-40 active reconciliation re-enqueued about 4,786 tokens per minute against worker capacity of about 1,364 checks per minute, starving priority-20 new-token checks. T40 records the v4.0.14 correction; verified-only Bonding semantics remain intact.
...
- A 2026-08-04 live counter audit found 193 production history rows through 16:05 UTC: 185 intervals peaked at 1 and 8 peaked at 2. Two tabs in the same in-app browser remained at 1, confirming cookie-based tab deduplication; no presence persistence or browser-console errors were observed and both PM2 processes had zero restarts over 16 hours.
