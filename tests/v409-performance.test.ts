...
test("VM release stays manual and creates neither backups nor GitHub Actions", async () => {
  ...
  assert.match(script, /node node_modules\/next\/dist\/bin\/next start/)
  assert.match(script, /kill -TERM "\$CANDIDATE_PID"/)
  assert.match(script, /Candidate port \$CANDIDATE_PORT is already in use/)
  assert.match(script, /Candidate did not release port \$CANDIDATE_PORT/)
  assert.match(script, /health\.version === process\.argv\[1\]/)
  assert.match(script, /SUPPORT_TURNSTILE_LOOPBACK_TEST_BYPASS="1"/)
